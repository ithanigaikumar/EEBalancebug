...
//   }
    ...
    //////////////////////////////////////////////////////////
    	...
    	uint16_t chunksInFrame = 0;
    	uint16_t emptyFrameHeader = 1;
    	int emptyFrame = 0x80000001;
    	int currentFrame[5000];
    	int emptycount;
  ...
  while(1){
	  ...
	  int in = getchar();
	   ...
	   while ((IORD(0x42000,EEE_IMGPROC_STATUS)>>8) & 0x7ff) { 	//Find out if there are words to read
		   int word = IORD(0x42000,EEE_IMGPROC_MSG);

		   if((word & 0x80000000) | (chunksInFrame==4999)){

			   fwrite(&chunksInFrame, 2, 1, ser);
