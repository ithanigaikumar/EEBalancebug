...
reg in_chunk; //to keep track if current pixel under evaluation at current clk posedge is within chunk

reg new_frame;
initial new_frame = 1'b0;
...
assign msg_buf_flush = ((s_chipselect & s_write & (s_address == `REG_STATUS) & s_writedata[4]) | (msg_buf_size == 10'd1022));
...
begin
   if (~reset_n) begin
	   s_readdata <= {32'b0};
		...
		if   (s_address == `REG_STATUS) s_readdata <= {14'b0,msg_buf_size,reg_status};
