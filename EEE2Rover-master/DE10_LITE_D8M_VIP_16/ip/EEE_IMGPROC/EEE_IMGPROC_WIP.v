...
always@(posedge clk) begin
	if (eop & in_valid & packet_video) begin  //Ignore non-video packets
		...
		if (frame_counter == 3'd5) begin
			sample <= 1'b1;
			frame_counter <= 3'd0;
			chunk_buffer_occupancy <= 8'b0;
			in_chunk <= 1'b0;
		end
		else sample <= 1'b0;
	end
...
always@(posedge clk) begin
	if(sample) begin
		if(white_detect & (!in_chunk) & in_valid) begin 
			...
			chunk_length <= chunk_length + 13'h1;
		end
		else if((!white_detect) & in_chunk & in_valid) begin
			in_chunk <= 0;
			if (chunk_buffer_occupancy == 8'h0) begin
				chunk_buffer[chunk_buffer_occupancy] <= {1'b1, x_start, y_start, chunk_length};
			end
			else begin
				chunk_buffer[chunk_buffer_occupancy] <= {1'b0, x_start, y_start, chunk_length};
			end
